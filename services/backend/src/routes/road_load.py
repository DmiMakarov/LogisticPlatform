...
import src.crud.road_load as crud
from src.auth.jwthandler import get_current_user
from src.schemas.road_load import RoadLoadOutSchema, RoadLoadInSchema
...
@router.get(
    "/notes",
    response_model=List[RoadLoadOutSchema],
...
@router.get(
    "/note/{note_id}",
    response_model=RoadLoadOutSchema,
    dependencies=[Depends(get_current_user)],
)
async def get_note(note_id: int) -> RoadLoadOutSchema:
    try:
...
@router.post(
    "/notes", response_model=RoadLoadOutSchema, dependencies=[Depends(get_current_user)]
)
